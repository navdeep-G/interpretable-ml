...
Subsequent sections will use simulated data to empirically demonstrate the desired relationships and interactions between input feature space $\mathbf{X}$, the output space $f(\mathbf{X}) = \mathbf{Y}$. Simulated data is generated using a pre-defined signal-generating function using four input features with interactions and with eight noisy features with ground truth containing three unique categories(class 0, class 1, class 2) as defined below,
\begin{equation}
\label{eq:f}
f = \text{num} _1 * \text{num}_4 + |\text{num}_8| * \text{num}_9^2 + e
\end{equation}

A $g_{\text{GBM}}$ is trained as the estimator to learn the patterns represented using multinomial logloss as the optimization function and softmax is used to generate the final predictions. 
...
The performance metric reference in Table \ref{eval-table} is for completeness. The goal of the paper is not to highlight the building the best possible model.
...
Given a learned function $g$ and a set of learned output responses $g(\mathbf{X}) = \mathbf{\hat{Y}}$, a surrogate decision tree $h_{\text{tree}}$ can be constructed to extract comprehensible and faithful concepts such that $h_{\text{tree}}(\mathbf{X}) \approx g_{\text{GBM}}(\mathbf{X}) \approx f(\mathbf{X})$:
...
\begin{figure}[H]
	...
	\caption{\textbf{a:} Illustrates $h_{\text{tree}}$ built using the original estimator $g_{\text{GBM}}$. Only the leaf nodes are colored, clearly highlighting class membership. The non-leaf nodes highlights the important features(namely num1, num4, 
	num9) which matches the other results throughout this text. \textbf{b:} Visualizes the decision stumps as a convenient text representation. The decision stumps highlight the rules learned by $h_{\text{tree}}$ for a single input row. The nested 
...
In the above mentioned learning task, the target labels are derived from the previously learned GBM response function $g_{\text{GBM}}$. \cite{dt_surrogate1} \cite{dt_surrogate2} provide other prescribed approaches for training a tree surrogate $h_{\text{tree}}$. 

\paragraph{Recommendations}
\begin{itemize}
\item In order to prevent $h_{\text{tree}}$ from creating overly complex un-interpretable "if-then-else" rules that do not generalize well, pruning(pre-pruning with cross validation or post-pruning by optimizing on the multi-class evaluation metic - e.g. f1-score, categorical cross entropy) can be applied to build stable trees.
\item $h_{\text{tree}}$ can elegantly handle high cardinality targets maintaining a high level of fidelity and faithfulness to the original  estimator function $g$. The faithfulness of the decisions generated by $h_{\text{tree}}$ depends on how precisely the tree surrogates captures the decisions learned by the original estimator.
\item Remember to handle class imbalance before fitting a $h_{\text{tree}}$ using an oversampling technique - e.g. Synthetic Minority Over-sampling Technique (SMOTE) \cite{chawla2002smote}, Adaptive synthetic sampling approach for imbalanced learning(ADASYN) \cite{he2008adasyn}.
\end{itemize}

\subsection{Decision Boundary Plots}

Decision boundaries are a hypersurface which provides information to about the characteristics of supervised multinomial classifiers by visualizing the distances of the data instances to a classifiers learned decision plane. The boundary helps in providing a direct reference to the complexity of a classifier and the dataset \cite{rao1997visualizing}, \cite{migut2015visualizing}. Figure 2 (a) and (d) illustrates different decision boundaries constructed using a GBM (Gradient Boosted Machine) and a MLP (Mulit-layer Perceptron classifier).
Such representation provides global explanations even high-cardinality targets using the two or three dimensional feature vectors selected from $X$. 
...
\begin{figure}[H]
	...
	\end{subfigure}%
	\captionsetup{font=footnotesize}
	\caption{The color of the data points correspond to the color of the supplied ground truth labels("class 0", "class 1", "class 2"). While the background color of the decision boundaries indicated class membership (i.e. separation and direction) as learned by the 
...
\begin{itemize}
	\item Decision boundaries are widely used, easily understandable 2D or 3D scatter plots well suited for helping to explain the behavior of multinomial classifiers \cite{migut2015visualizing}. 
	\item Decision boundaries are able to show data and class membership in original feature space which is helpful in understanding feature interaction.
	\item Decision boundaries are model agnostic and work well for any form of simple or complex ("black-box") model enabling effective model comparison.
	\item Decision boundaries provide the ability to instantly identify critical and cost changing data points, e.g. those data instances closer to the decision boundary.
	\item How do we handle high dimensional feature space \\ $\mathcal{X} \subset \mathbb{R}^P$?
	\begin{itemize}
		\item A small multiples plot of 2D/3D decision boundaries showing many combinations of the most important features. Top features could be selected using the global Shapely feature importance.
