...
\begin{figure}[htb]
	\begin{subfigure}[tb]{.5\textwidth}
		\begin{center}
			\includegraphics[width=\linewidth]{img/sdt_tree}
			\caption{}
	...
	\begin{subfigure}[tb]{.5\textwidth}
		\begin{center}
			\includegraphics[width=1.3\linewidth]{img/decision_stumps_txt.png}
			\caption{}
			\label{fig:surrogate_dt}
		\end{center}
	\end{subfigure}%
	...
	which matches the results computed using the Shapely Feature ranking. \textbf{b:} Visualizes the decision stumps as a convenient text representation}
\end{figure}

In the above mentioned learning task, the target labels are derived from the base estimators learned function. \cite{dt_surrogate1} \cite{dt_surrogate2} provide detained information for training $h_{\text{tree}}$. 
...
\item SDT can elegantly handle high cardinality targets maintaining high level of fidelity and faithfulness to the Oracle(base estimator function $g$)
\item Remember to handle class imbalance before fitting a SDT using an oversampling technique - e.g. Synthetic Minority Over-sampling Technique(SMOTE) \cite{chawla2002smote}, Adaptive synthetic sampling approach for imbalanced learning(ADASYN) 
\item Scope of Interpretation: Global and Local
\cite{he2008adasyn}
\end{itemize}
...
\begin{figure}[htb]
	...
	GBM(\textbf{Figures a, b, c})/MLP classifier(\textbf{Figure d:} Visualizes the decision boundary learned by a multi-layered MLP classifier, highlighting a different shape of the decision boundary as compared to GBM model(tree based models learn rectangular boundaries.}
\end{figure}
