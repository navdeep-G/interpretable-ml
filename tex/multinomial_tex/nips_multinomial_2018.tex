...
Decision Boundaries is an informative interpretation technique to understand the characteristics of multiclass predictive model by visualizing the distances of the data elements to a model's learned decision plane. The boundary helps in providing a direct reference to the complexity of a classifier and the dataset. Figure 2 (a) and (d) illustrates different decision boundaries constructed using a GBM(Gradient Boosted Machine) and a MLP(Mulit-layer Perceptron classifier).
...
\begin{figure}[htb]
	...
	\begin{subfigure}[tb]{.5\textwidth}
		...
		\end{center}
	\end{subfigure}%
	\caption{The color of the data points correspond to the color of the supplied ground truth labels. While the background color of the decision boundaries indicated class membership as learned by the estimator}
\end{figure}
...
\begin{figure}[!htbp]
	\begin{center}
		\includegraphics[height=118pt]{img/pdp_ice_sim_annotated.png}
